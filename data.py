...
class Data:
    ...
    def initialize_labels_with_idxs(self, idxs):
...
def get_MNIST(handler):
    raw_train = datasets.MNIST('./data/MNIST', train=True, download=False)
    raw_test = datasets.MNIST('./data/MNIST', train=False, download=False)
...
def get_FashionMNIST(handler):
    ...
    #return Data(raw_train.data[:40000], raw_train.targets[:40000], raw_test.data[:40000], raw_test.targets[:40000], handler)
    return Data(raw_train.data[:10500], raw_train.targets[:10500], raw_test.data[:8400], raw_test.targets[:8400], handler)
...
#新加入的函数，用来获取老师这次给的数据集
def get_BDIC(handler):
    path = './data/BDIC/'
    X_train = torch.load(path + 'train_images.pt')[:10500]
    Y_train = torch.load(path + 'train_labels.pt')[:10500]
    X_test = torch.load(path + 'valid_images.pt')#还是val？？
    Y_test = torch.load(path + 'valid_labels.pt')
